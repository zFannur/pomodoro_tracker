...
**Десктопный Pomodoro-таймер и задачник для Windows, дружащий с Obsidian.**
...
спринта, Flowtime). История помидоров и спринты — обычные `.md`-файлы в
Obsidian-валте; задачи живут в `tasks.json` и зеркалятся в валт для просмотра.
...
[![Tests](https://img.shields.io/badge/tests-38%20passing-3FA45B.svg)](#тесты)
...
**Задачи** — в `tasks.json` (`%APPDATA%\com.local\pomodoro_tracker\`,
атомарная запись, стабильные id): задачник не зависит от валта и работает,
даже если валт недоступен. При первом запуске задачи автоматически
мигрируют из старого `Задачи.md`.

В папке хранения (по умолчанию `…\Obsidian\Помодоро`) остаются:

```
Помодоро/
├── Задачи.md                    # ЗЕРКАЛО задачника (только просмотр)
...
### Задачи.md — одностороннее зеркало

Перегенерируется при каждом изменении задач: видно в Obsidian, работает
поиск и git-история, это же аварийный бэкап. **Правки в нём не читаются**
(сверху — пометка об этом). Выключатель зеркала — в настройках.
...
Там же `tasks.json` (задачи) и `timer_state.json` — состояние таймера для
восстановления.
...
│   ├── json_task_repository.dart# tasks.json + миграция + зеркало в валт
│   ├── vault_repositories.dart  # файловый доступ, инбокс, атомарная запись
...
- `test/strings_test.dart` — переключение языка интерфейса, локализованные
  единицы времени.
- `test/json_task_repository_test.dart` — миграция из `Задачи.md`, round-trip
  `tasks.json`, разводка дубликатов id, зеркало, файл-инбокс.
...
- **`Задачи.md` — только зеркало**: правки в нём не читаются вообще.
  Добавлять задачи из Obsidian — через `Входящие.md`; переносить и
...
- Сознательно без серверных фич: аккаунтов, лидерборда, шаринга,
  интеграций (Slack/Trello/Todoist/Google), почтовых отчётов, повторяющихся
  задач — это локальный инструмент, а не SaaS-платформа.
